...
class EntityResolver:
    ...
    def compute_age(self, birth_date: str, death_date: str = None,
    ...
    def validate_age_consistency(self, extracted_age: int, birth_date: str,
                                  death_date: str) -> dict:
    ...
    def link_family_members(self, records: list) -> dict:
        ...
        for record in records:
            for field in ["child_name", "father_name", "mother_name",
                         "groom_name", "bride_name", "deceased_name"]:
    ...
    def _detect_relationship(self, member1: dict, member2: dict) -> dict | None:
        ...
        if member1["role"] == "father_name" and member2["role"] == "child_name":
            return {
                "type": "parent_child", "parent": member1["name"],
                "child": member2["name"], "parent_role": "father", "confidence": 0.8
            }
        if member2["role"] == "father_name" and member1["role"] == "child_name":
            return {
                "type": "parent_child", "parent": member2["name"],
                "child": member1["name"], "parent_role": "father", "confidence": 0.8
            }
        if member1["role"] == "mother_name" and member2["role"] == "child_name":
            return {
                "type": "parent_child", "parent": member1["name"],
                "child": member2["name"], "parent_role": "mother", "confidence": 0.8
            }
        if member2["role"] == "mother_name" and member1["role"] == "child_name":
            return {
                "type": "parent_child", "parent": member2["name"],
                "child": member1["name"], "parent_role": "mother", "confidence": 0.8
            }
        if (member1["role"] == "groom_name" and member2["role"] == "bride_name" and
            member1["record"] == member2["record"]):
            return {
                "type": "spouse", "person1": member1["name"],
                "person2": member2["name"], "confidence": 0.9
            }
        return None
    ...
    def resolve_entity(self, record: dict) -> dict:
        ...
        if all(k in record for k in ["birth_date", "death_date"]):
            birth = record["birth_date"].get("value", "") if isinstance(record["birth_date"], dict) else ""
            death = record["death_date"].get("value", "") if isinstance(record["death_date"], dict) else ""
            ...
            if birth and death and birth != "Unknown" and death != "Unknown":
                ...
                if "age" in record:
                    extracted_age = record["age"].get("value") if isinstance(record["age"], dict) else None
                    if extracted_age:
                        try:
                            age_int = int(extracted_age)
                            resolved["age_validation"] = self.validate_age_consistency(age_int, birth, death)
                        except (ValueError, TypeError):
                            pass
