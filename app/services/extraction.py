...
class InformationExtractor:
    ...
    def extract_birth_record(self, text: str) -> dict:
        child_name, child_conf = self.extract_field(text, self.birth_patterns["child_name"], use_multiline=True)
        ...
        try:
            ...
            for roman, month_num in roman_map.items():
                if f'.{roman}.' in date_str.upper() or f'.{roman}' in date_str.upper():
                    match = re.search(r'(\d{1,2})\.' + roman + r'\.?(\d{4})', date_str, re.IGNORECASE)
                    if match:
                        day, year = int(match.group(1)), match.group(2)
                        return f"{year}-{month_num:02d}-{day:02d}"
    ...
    def extract(self, text: str) -> dict:
        ...
        logger.info(f"Extraction: {record['record_type']} (avg={avg_confidence:.2f}, review={record['needs_review']})")
        return record
